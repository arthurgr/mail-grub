...
public class PackagingController {
  ...
  public @ResponseBody PagedResponse<Packaging> getPackaging(
    ...
    Page<Packaging> pageResult =
        (packagingMaterials != null && !packagingMaterials.trim().isEmpty())
            ? packagingRepository.findByPackagingMaterialsContainingIgnoreCase(
                packagingMaterials.trim(), pageable)
  ...
  public @ResponseBody String addNewPackaging(@RequestBody Packaging packaging) {
    if (packaging.getAverageCost() != null
        && packaging.getQuantity() != null
        && packaging.getQuantity() != 0) {
  ...
  public @ResponseBody String updatePackaging(
      @PathVariable Integer id, @RequestBody Packaging updatedPackaging) {
    return packagingRepository
        .findById(id)
        .map(
            packaging -> {
              if (updatedPackaging.getPackagingMaterials() != null)
                packaging.setPackagingMaterials(updatedPackaging.getPackagingMaterials());
              if (updatedPackaging.getAverageCost() != null)
                packaging.setAverageCost(updatedPackaging.getAverageCost());
              if (updatedPackaging.getQuantity() != null)
                packaging.setQuantity(updatedPackaging.getQuantity());
              if (updatedPackaging.getProcurement() != null)
                packaging.setProcurement(updatedPackaging.getProcurement());
              packagingRepository.save(packaging);
