  ...
  public @ResponseBody PagedResponse<RecipeResponse> getAllRecipes(
    ...
    List<RecipeResponse> content =
        pageResult.getContent().stream()
            .map(
                recipe -> {
                  List<RecipeResponse.IngredientEntry> ingredients =
            ...
            .map(
                entry -> {
                  Ingredient ingredient =
                      ingredientRepository.findById(entry.getIngredientId()).orElseThrow();
        ...
        .map(
            recipe -> {
              ...
              if (updateRequest.getIngredients() != null
                  && !updateRequest.getIngredients().isEmpty()) {
                recipe.getRecipeIngredients().clear();

                List<RecipeIngredient> updatedIngredients =
                    updateRequest.getIngredients().stream()
                        .map(
                            entry -> {
                              Ingredient ingredient =
                                  ingredientRepository
                                      .findById(entry.getIngredientId())
                                      .orElseThrow();
                              RecipeIngredient ri = new RecipeIngredient();
                              ri.setRecipe(recipe);
                              ri.setIngredient(ingredient);
